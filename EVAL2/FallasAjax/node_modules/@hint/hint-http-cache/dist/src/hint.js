    ...
    constructor(context) {
        ...
        const hasInvalidDirectives = (directives, fetchEnd) => {
            ...
            if (invalidDirectives.size > 0) {
                const message = i18n_import_1.getMessage('directiveInvalid', context.language, [
            ...
            if (invalidValues.size > 0) {
                const message = i18n_import_1.getMessage('directiveInvalidValue', context.language, [
        ...
        const hasNoneNonRecommendedDirectives = (directives, fetchEnd) => {
            ...
            if (nonRecommendedDirective) {
                const message = i18n_import_1.getMessage('directiveNotRecomended', context.language, nonRecommendedDirective);
        ...
        const validateDirectiveCombinations = (directives, fetchEnd) => {
            ...
            if (usedDirectives.has('no-cache') || usedDirectives.has('no-store')) {
                ...
                if (hasMaxAge) {
                    const message = i18n_import_1.getMessage('wrongCombination', context.language, header);
        ...
        const hasSmallCache = (directives, fetchEnd) => {
            ...
            if (!isValidCache) {
                const message = i18n_import_1.getMessage('targetShouldNotBeCached', context.language, [maxAgeTarget, header]);
                context.report(fetchEnd.resource, message, {
                    codeLanguage: 'http',
                    codeSnippet: `Cache-Control: ${header}`,
                    severity: utils_types_1.Severity.warning
        ...
        const usesFileRevving = (directives, fetchEnd) => {
            ...
            if (!matches) {
                const message = i18n_import_1.getMessage('noCacheBustingPattern', context.language, resource);
        ...
        const hasLongCache = (directives, fetchEnd) => {
            ...
            if (usedDirectives.has('no-cache') || !longCache) {
                const message = i18n_import_1.getMessage('staticResourceCacheValue', context.language, [maxAgeResource, header]);
            ...
            if (!immutable) {
                const message = i18n_import_1.getMessage('staticNotImmutable', context.language, header);
